...
│       │   │   │   │   │   ├── sessions/    # Сессии кампании
│       │   │   │   │   │   ├── session/[id]/scene-editor/ # Визуальный редактор дерева сцен
...
│   ├─ campaigns                 — CRUD кампаний (привязка к сеттингу)
│   ├─ campaigns/{id}/sessions   — Сессии кампании (CRUD, порядок)
│   ├─ campaigns/{id}/sessions/{sid}/scenes — Дерево сцен сессии (CRUD, дерево, связи)
...
/display/*                       — Display client endpoints
│   ├─ current-scene             — Текущая сцена (polling fallback)
│   └─ join/{campaign}           — Подключение к кампании
...
## Content Architecture — Правила → Сеттинг → Кампания → Сессии → Сцены
...
├─────────────────────────────────────────────────────────────────┤
│  📅 СЕССИЯ (Session) → принадлежит кампании                    │
...
│   │   ├─ Сессия #1: "Прибытие"
│   │   │   ├─ Intro → Поезд молний → Шарн вокзал → Таверна "Сломанная наковальня"
│   │   │   └─ [Выбор] → Пойти в нижний город / Пойти в верхний город
│   │   └─ Сессия #2: "Нижний Шарн"
│   │       └─ ...дерево сцен...
│   └─ Расы: Человек, Эльф, Дварф, Полурослик, Перевёртыш, Калаштар, Кованый
│           (Кованый и Перевёртыш — уникальные для Eberron!)
│
...
├─ Другие характеристики (WS, BS, S, T, W, I, A, Ld)
├─ Другие кубики (d6, d100, без d20)
├─ Другие формулы боя
├─ Сеттинг: Imperium of Man
│   ├─ Расы: Человек (Имперский), Эльдар, Тау, Орк
│   ├─ Классы: Гвардеец, Техножрец, Псайкер
│   └─ Телекинез — пси-способность (override: нет компонентов, другие правила)
```
...
CREATE TABLE sessions (
    ...
    name JSONB NOT NULL,                  -- {"ru": "Прибытие в Шарн"}
...
```
📋 Backoffice → Кампания → Сессия #1 → Редактор сцен

┌──────────────────────────────────────────────────────────────┐
│  📅 Сессия #1: "Прибытие в Шарн"          [+ Добавить сцену]│
│                                                              │
│  ┌─────────┐    ┌─────────────────┐    ┌───────────────┐    │
...
│   │   ├─ Игроки (приглашённые)
│   │   ├─ Сессии
│   │   │   ├─ #1 "Прибытие" → [Редактор дерева сцен]
│   │   │   ├─ #2 "Нижний Шарн" → [Редактор дерева сцен]
│   │   │   └─ [+ Новая сессия]
│   │   ├─ NPC
...
- Relations: belongsTo Setting, User (DM); hasMany Sessions, Characters, Encounters, Notes, RandomTables; belongsToMany Users (players)
- Наследует RuleSystem через `setting.rule_system_id`

**Session** — Игровая сессия
- Translatable: `name`, `summary`
- Fields: `campaign_id`, `number` (порядковый), `status` (planned/active/completed), `played_at`, `sort_order`
- Relations: belongsTo Campaign; hasMany Scenes
...
- Sample кампания в сеттинге Eberron с несколькими сессиями
...
6. DM: Session management (список сессий кампании, CRUD)
